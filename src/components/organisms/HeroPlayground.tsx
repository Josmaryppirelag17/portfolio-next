...
export default function HeroPlayground({
  ...
  useEffect(() => {
    ...
    let animId: number;
    const shape = getShapeData(activeShape);
    ...
    const draw = () => {
      ...
      angleRef.current.y += 0.005 * rotationSpeed + mouseRef.current.x * 0.0001;
  ...
  }, [activeShape, accent, rotationSpeed, wireframeScale, reducedMotion]);
      ...
      <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 lg:grid-cols-12 gap-12 items-center relative z-10">
        ...
        <div className="lg:col-span-7 flex flex-col space-y-6">
          ...
          <div className="flex items-center space-x-3">
            ...
            {/* Built-in Sound Master Controller (hidden on mobile — duplicated in navbar) */}
            <button
              onClick={masterToggle}
              aria-label={t("audio_tooltip")}
              className={`max-sm:hidden ml-auto flex items-center space-x-2 px-3 py-1.5 rounded-md border-2 text-xs font-mono transition-all duration-300 ${
                audioOn
        ...
        <div className="lg:col-span-15 xl:col-span-5 h-[500px] flex flex-col justify-between p-6 bg-[#111232]/95 border-4 border-[#111232] rounded-xl neo-brutal-border-cyan relative overflow-hidden group">
          ...
          >
            ...
            {balloonCount > 0 && (
              <div
                className="absolute p-2 pointer-events-none rounded-md bg-brand-pink text-brand-bg font-display text-[10px] uppercase border border-brand-bg z-20 animate-[pop-in_0.4s_ease-out_forwards]"
                key={balloonCount}
              >
                {t("pop_synth_note")}
              </div>
          ...
          <div className="border-t border-brand-cyan/15 pt-4 flex flex-col space-y-3 relative z-10 bg-brand-bg/85 p-3 rounded-lg">
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex space-x-2" role="group" aria-label="Selector de color de acento">
                {(["pink", "cyan", "lime"] as AccentTheme[]).map((theme) => (
                  <button
                    ...
                    aria-label={`Color ${theme === "pink" ? "rosa" : theme === "lime" ? "lima" : "cian"}`}
                    aria-pressed={accent === theme}
                    className={`rounded-full border border-brand-bg cursor-pointer transition-transform duration-200 hover:scale-125 min-w-[44px] min-h-[44px] flex items-center justify-center ${
                      theme === "pink"
                        ? "bg-brand-pink"
                        : theme === "lime"
                          ? "bg-brand-lime"
