...
function formatSynthKeyAria(template: string, key: SynthKey): string {
  return template
...
export default function ContactTerminal() {
  ...
  const synthScale: SynthKey[] = [
    ...
    { note: "G4", freq: 392.0, label: "G", colorClass: "bg-brand-pink text-white" },
    { note: "A4", freq: 440.0, label: "A", colorClass: "bg-brand-cyan text-brand-bg" },
  ...
  const onSubmit = async (data: ClientContactInput) => {
    setTerminalLines([
      "> Initializing transmitter uplink...",
      "> POSTing info payload to /api/contact...",
    ]);
      ...
      if (!response.ok) {
        ...
        if (httpStatus === 429) {
          const retry = errorData.error?.details?.retryAfter
            ...
            : "";
          throw new Error(`Límite de envíos alcanzado (rate limit).${retry}`);
        }
        const errMsg =
          errorData.error?.message ||
          errorData.errors?.join(" | ") ||
          `Error del servidor (HTTP ${httpStatus})`;
          ...
          <div className="lg:col-span-12 xl:col-span-5 flex flex-col justify-between p-6 bg-[#111232]/95 border-4 border-[#111232] rounded-xl neo-brutal-border-lime relative overflow-hidden">
            ...
            <div className="space-y-6 relative z-10">
              <div className="flex items-center justify-between border-b border-brand-lime/20 pb-4">
                ...
                <span className="font-mono text-[8.5px] text-brand-pale/50 uppercase">
                  ASTATIC_FREQ: OK
                </span>
              </div>
              ...
              <p className="font-sans text-xs text-brand-pale/85 leading-relaxed text-left">
                {t("synth_explanation") ||
                  "Enjoy a fully synthesized custom piano block! Clicking on raw key nodes triggers polyphonic spatial soundwaves using native low-pass web filters."}
              </p>
              ...
              >
                ...
                <div
                  ...
                  onKeyDown={(e) => {
                    ...
                    const currentIndex = Array.from(keys).indexOf(
                ...
                <div className="mt-4 pt-4 border-t border-brand-lime/10">
                  <div className="font-mono text-[9px] text-brand-pale/50 uppercase tracking-widest mb-2 flex items-center justify-between">
                    ...
                    <span className="text-brand-lime font-bold">
                      OSC_{activeWave.toUpperCase()}
                    </span>
                  </div>
                  <div className="grid grid-cols-4 gap-1">
                    {(["triangle", "sine", "sawtooth", "square"] as OscillatorType[]).map(
                      (wave) => (
                        <button
                          key={wave}
                          type="button"
              ...
              <div className="p-3 bg-brand-bg/40 border border-brand-pale/5 rounded-md text-left">
                ...
                <div className="font-mono text-[9px] text-brand-pale/60 leading-normal text-left">
                  OSCILLATORS: MOD_{activeWave.toUpperCase()}_WAVE
                  <br />
                  GAIN_ATTENUATION: LOG_EXPONENTIAL_DECAY
                  <br />
                  ...
                  <div className="space-y-4">
                    ...
                    >
                      ...
                      <input
                        ...
                        name="fax"
                        onChange={(e) => {
                          faxValue.current = e.target.value;
                        }}
                        tabIndex={-1}
                        autoComplete="off"
                      />
                      ...
                      <input
                        ...
                        name="website"
                        onChange={(e) => {
                          websiteValue.current = e.target.value;
                        }}
                        tabIndex={-1}
                        autoComplete="off"
                      />
                    ...
                    <div className="space-y-1 text-left">
                      <label
                        ...
                        className="block font-mono text-[10px] text-brand-pale/70 uppercase text-left"
                      >
                        {t("contact_name")}
                      </label>
                      <input
                      ...
                      />
                      {errors.name && (
                        <p className="font-mono text-[9px] text-brand-pink mt-1">
                          {errors.name.message}
                        </p>
                      )}
                    </div>
