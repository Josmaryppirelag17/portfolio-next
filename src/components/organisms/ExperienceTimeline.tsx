      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        ...
        {experience.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 space-y-6 text-center">
            <div className="h-20 w-20 rounded-full bg-brand-bg border-4 border-brand-cyan/30 flex items-center justify-center">
              <XCircle size={40} className="text-brand-cyan/60" />
            </div>
            <div className="space-y-2">
              <p className="font-mono text-sm text-brand-cyan font-bold uppercase tracking-widest">
                {t("empty_experience")}
              </p>
              <p className="font-sans text-xs text-brand-pale/60 max-w-md mx-auto leading-relaxed">
                {t("empty_experience_desc")}
              </p>
            </div>
            <div className="font-mono text-[8px] text-brand-pale/45 uppercase tracking-widest bg-brand-bg px-4 py-2 rounded border border-brand-pale/10">
              STATUS_CODE: 0x404 // NO_MILESTONES_FOUND
            </div>
          </div>
        ) : (
            ...
            {experience.map((milestone, idx) => {
              ...
              return (
                <div
                  ...
                  aria-label={`${milestone.period}: ${milestone.role} at ${milestone.company}`}
                >
                  {/* Timeline node bullet point spark */}
                  <div
                    className={`absolute left-[-32px] md:left-[-48px] top-1.5 h-6 w-6 rounded-full border-4 border-[#111232] transition-all duration-300 flex items-center justify-center ${
                      isHovered
                    ...
                    <div className="py-4 space-y-3 text-left">
                      ...
                      <div className="pl-4 border-l border-brand-cyan/20 space-y-2">
                        {milestone.bullets.map((bullet, bIdx) => (
                          <div
                            key={bIdx}
                            className="flex items-start space-x-2 text-xs font-sans text-brand-pale/80 max-w-2xl leading-relaxed text-left"
                          >
