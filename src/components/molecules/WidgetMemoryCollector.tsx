...
export default function WidgetMemoryCollector() {
  ...
  const [logs, setLogs] = useState<string[]>(["// SYS STATS LOADED OK.", "// STANDBY CACHE READY FOR SCRUB."]);
  ...
  const runGarbagePurge = () => {
    ...
    steps.forEach((step, idx) => {
      setTimeout(() => {
        ...
        if (idx === steps.length - 1) { setPurging(false); soundEngine.playSuccess(); }
      ...
      <div className="flex-grow w-full bg-[#090b1c] rounded border border-brand-pale/5 flex p-2.5 space-x-3 items-center mb-1.5 overflow-hidden">
        <div className="relative shrink-0 flex items-center justify-center">
          <div className="w-14 h-14 rounded-full border-4 border-brand-bg flex items-center justify-center text-[10px] font-mono font-bold text-white relative">
            <svg className="absolute inset-0 transform -rotate-90" viewBox="0 0 36 36">
              ...
              <path className="text-brand-lime transition-all duration-300" strokeDasharray={`${percent}, 100`} strokeWidth="3.5" strokeLinecap="round" stroke="currentColor" fill="none" d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831" style={{ filter: "drop-shadow(0px 0px 4px #DCF10B)" }} />
            </svg>
        ...
        <div className="flex-grow h-14 overflow-hidden flex flex-col justify-end">
          {logs.slice(0, 3).map((line, lid) => (
            <div key={lid} className="font-mono text-[7px] text-brand-pale/60 tracking-wider truncate mb-0.5">{line}</div>
          ))}
        </div>
      </div>
      <button onClick={runGarbagePurge} disabled={purging}
