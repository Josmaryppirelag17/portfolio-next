...
export default function WidgetRetroTerminal({ onTriggerGlitch, onTriggerOverload, onCalmReactor }: RetroTerminalProps) {
  const [inputVal, setInputVal] = useState("");
  const [history, setHistory] = useState<string[]>(["SYSINIT_COMPLETE // CHIP ONLINE.", "WRITE 'help' TO QUERY CMD SCRIPTS."]);
  ...
  const handleCommandSubmit = (e: React.FormEvent) => {
    ...
    if (cleanCmd === "help") {
      newHistory.push("LIST OF PERMITTED CONSOLE PROMPT SCRIPTS:", " - 'glitch' : Simulate CSS chromatic aberration glitch lines", " - 'laser'  : Emit monophonic musical octave sweeping notes", " - 'overload' : Initiate core reactor uranium overdrive", " - 'calm'    : Emergency vent reactor rods to 40%", " - 'status'  : Query cognitive host and cybernetic diagnostics", " - 'clear'   : Wipe log buffers");
      ...
      newHistory.push("TELEMETRY_STATUS:", " - HOST_STATUS: COMPILER_STABLE", " - AVATAR: SKELETAL_CCDIK_ARM_V3.1", " - SOUND: RETRO_POCKET_OSC", " - LOCATION: MARACAIBO // MCBO HUB");
    } else if (cleanCmd === "clear") {
      setHistory([]); setInputVal(""); return;
    } else {
  ...
  return (
    <WidgetShell title="CYBERNETIC CODES CONSOLE" icon={Terminal} iconColor="text-brand-pink" status="[HOST_TERM]">
      <div ref={containerRef} className="flex-grow w-full h-[85px] rounded bg-[#090b1c] border border-brand-pink/15 p-2 font-mono text-[7px] text-brand-pale/80 overflow-y-auto mb-2 relative scroll-smooth selection:bg-brand-pink">
        {history.map((line, index) => (
          <div key={index} className="leading-relaxed mb-0.5 tracking-wider font-light">{line}</div>
        ))}
      </div>
      <form onSubmit={handleCommandSubmit} className="flex gap-1.5 z-10">
        ...
        <input type="text" value={inputVal} onChange={(e) => setInputVal(e.target.value)} placeholder="Type 'help'..."
