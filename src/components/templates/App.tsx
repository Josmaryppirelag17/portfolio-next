...
export default function App() {
  ...
  useEffect(() => { capturePageView(); }, [language]);
  useEffect(() => { document.documentElement.classList.toggle("reduce-motion-override", reducedMotion); }, [reducedMotion]);
  ...
  return (
    <div className="min-h-screen bg-brand-bg text-[#DBEAEC] font-sans selection:bg-brand-pink selection:text-white">
      <a href="#main-content" className="fixed left-0 top-0 z-50 -translate-y-full bg-brand-pink px-4 py-2 text-black font-mono text-xs transition-transform focus:translate-y-0">{t("skip_to_content")}</a>
      ...
      />
      <main id="main-content" className="relative" aria-label={t("main_content_label") || "Main content"}>
        ...
        <Suspense fallback={<SectionFallback />}><ExperienceTimeline /></Suspense>
        <Suspense fallback={<SectionFallback />}><ProjectsShowcase /></Suspense>
        <Suspense fallback={<SectionFallback />}><InteractiveSkills /></Suspense>
        <Suspense fallback={<SectionFallback />}><ContactTerminal /></Suspense>
      </main>
