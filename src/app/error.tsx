...
export default function Error({ reset }: { error: Error; reset: () => void }) {
  return (
    <div role="alert" className="flex min-h-screen flex-col items-center justify-center gap-4 bg-[#111232] px-6">
      ...
      <button onClick={reset} aria-label="Recargar página" className="cursor-pointer rounded border border-brand-cyan/40 px-4 py-2 font-mono text-xs text-brand-cyan transition-colors hover:border-brand-cyan hover:bg-brand-cyan/10">
        RELOAD
      </button>
    </div>
  );
